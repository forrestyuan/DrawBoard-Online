...
export default class DrawBoard{
  ...
  mouseXY(event) {
	 ...
	 */
	syncData(axis) {
		let data = JSON.stringify({"username":sessionStorage.getItem('drawusername'),axis});
	...
	drawEvent(isUnbind) {
		...
		this.canvas[eventEnd] = e => {
			this.canvas[eventMove] = null;
			this.socket.emit('canSetBeginPath',JSON.stringify({"username":sessionStorage.getItem('drawusername')}));
			this.pushStack();
		};
	};
	//画布历史穿梭（前进和后退）

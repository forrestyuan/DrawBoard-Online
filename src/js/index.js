...
const T = new require('../utils/index');
...
class DrawBoard{
	...
	constructor(obj){
		...
		this.ctx = obj.ctx;
		this.winW = T.getTargetWH()[0]; //屏幕宽
		this.winH = T.getTargetWH()[1]; //屏幕高
	...
	updateCtxStyle(obj){
		...
		this.canvas.style.backgroundColor = obj.canvasColor ||  this.canvas.style.backgroundColor;
	}
...
function scaleHandler(dprBox, obj, isLarge){
	if(isLarge){
		dprBox.value = (+dprBox.value + 0.1).toFixed(1);
		obj.scaleList[1] = (+ dprBox.value);
		if(obj.scaleList[1] > 5){
			obj.scaleList[1]=5;
			dprBox.value=5;
			return;
		}
		...
		dprBox.value = (+dprBox.value - 0.1).toFixed(1);
		obj.scaleList[1] = (+ dprBox.value);
		if(obj.scaleList[1] < 0.1){
			obj.scaleList[1]= 0.1;
			dprBox.value=0.1;
			return;
		}
	}
	obj.canvas.style.width = obj.canvasW * obj.scaleList[1]+'px';
	obj.canvas.style.height = obj.canvasH * obj.scaleList[1]+'px';
	obj.updateParam();
	console.log(obj.drawLayerLeft)
}
...
window.onload = function() {
  ...
  let canvas = document.getElementById("canvas");
	...
	let db = new DrawBoard({ canvas: canvas, ctx: ctx , penceilWeight: penceilWeight});
	init(db);

	T.getEle("#backward").onclick = function(){
		db.travel(-1)
	}
	T.getEle("#forward").onclick = function(){
		db.travel(1)
	}
	T.getEle("#clearAll").onclick = function(){
		db.ctx.clearRect(0,0,db.canvasW,db.canvasH);
		db.timeTravelStep = -1;
		db.drawHistoryStack = [];
	}
	...
	T.getEle("#larger").onclick=function(){
		scaleHandler(scaleNum,db,true);
	}
	T.getEle("#smaller").onclick = function(){
		scaleHandler(scaleNum,db,false);
	}
	T.getEle('.shareLayerBtn').onclick = function(){
		showPanel = !showPanel;
